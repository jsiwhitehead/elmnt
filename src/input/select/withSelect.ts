import { enclose, HOC, memoize, methodWrap } from 'mishmash';
...
export default enclose(
  ({ setState }) => {
    ...
    return (props, { activeIndex, isOpen, noScrollRef }) => {
      const {
        isList,
        value,
        onChange,
      ...
      const scrollToIndex = (index, padding) => {
        if (scrollElem) {
          if (!scrollElem.offsetHeight) {
            setState({ noScrollRef: true }, () =>
              setState({ noScrollRef: false }),
            );
          } else {
      ...
      return {
        ...
        activeIndex,
        labels: memoize(newLabels),
        labelIndices: memoize(labelIndices),
        labelText: !isList
  ...
  { activeIndex: 0, isOpen: false, noScrollRef: false },
) as HOC;
