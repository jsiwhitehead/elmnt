...
const formatFloat = (value, { point = false, zeros = 0 }: FloatConfig) =>
  `${value === null ? '' : `${value}`}${point ? '.' : ''}${Array(
    zeros + 1,
  ).join('0')}`;
parsers.float = new Parser<number, FloatConfig>(
  text => {
...
parsers.date = new Parser<Date, DateConfig, DateExtraProps>(
  (text, { noDay }) => {
    const v = text.replace(/[^\d\/]/g, '');
    const dateSplit = v.split('/');
    if (dateSplit.length === (noDay ? 2 : 3)) {
      const [dStr, mStr, yStr] = noDay
  ...
  },
  {
    format: (
      value,
      { padDate = true, padMonth = true, fullYear = false },
      { noDay },
    ) => {
